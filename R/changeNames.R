...
setMethod('changeNames', signature = c ('BioData'),
	...
	if ( what=='row' ) {
		...
		if ( length(which(table(as.character(x$annotation[,colname] ) ) > 1 ) ) > 0 ){
			...
			rownames(x$annotation) <- x$annotation[ , x$rownamescol ] <- x$forceAbsoluteUniqueSample( as.character(x$annotation[,colname] ) )
		}else {
		...
		if ( length(which(table(as.character(x$samples[,colname] ) ) > 1 ) ) > 0 ){
			...
			rownames(x$samples) <- x$samples[ , x$sampleNamesCol ] <- x$forceAbsoluteUniqueSample( as.character(x$samples[,colname] ) )
