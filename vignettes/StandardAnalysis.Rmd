...
test <- as_BioData( m )
test
reduceTo(test, what='row', to=rownames(test)[1:5])
test
```
...
This will merge the raw data and all entries in the annotation and samples tables. In addition data in the usedObj list is stored.
It will drop all normalized and z.scored data as these steps have to be re-calculated for the merged dataset.
...
I will go into more details for our merged dataset here:

```{r}{
mds(merged)
## the 3D data is stored in merged$usedObj$
merged$usedObj$MDS
normalize(merged)
